      ...
      <div class="p-[3rem] w-[100%]">
        ...
        <div class="w-full flex flex-row flex-wrap">
          <template v-for="card in cards" :key="card.id">
            <div class="flex flex-col justify-center items-center mr-[3rem] mt-[3rem] border border-darkGrey rounded-sm px-[1rem] py-[2rem]">
              <div class="w-full flex flex-row justify-between items-center">
                <div v-for="(tradeCard, index) in card.tradeCards" :key="index" class="flex flex-row">
                  <div v-if="tradeCard.type === 'OFFERING'" class="ml-[0.5rem] mr-[0.5rem]" >
                    <div v-if="tradeCard.card.name.length > 0">
                      <Card :name="tradeCard.card.name" :imageUrl="tradeCard.card.imageUrl" />
                    </div>
                  </div>
                  <div v-else-if="tradeCard.type === 'RECEIVING'" class="mr-[0.5rem] ml-[0.5rem]" >
                    <div v-if="tradeCard.card.name.length > 0">
                      <Card :name="tradeCard.card.name" :imageUrl="tradeCard.card.imageUrl" />
                    </div>
                  </div>
              ...
              <div class="mt-[2rem]" v-if="card.userId === userId">
                <button @click="selectCard(card.id)" class="flex flex-row items-center justify-center bg-darkGrey h-[3rem] w-[8rem] rounded-sm text-[1rem] text-white font-medium">
      ...
      <div v-if="isModalOpen" class="fixed z-10 inset-0 overflow-y-auto bg-gray-800 bg-opacity-50">
        <div class="flex items-center justify-center mt-[10%]">
          <div class="relative bg-white w-[50rem] min-h-[30rem] p-[3rem] rounded shadow-lg">
            ...
            <div>
              <p class="text-[1.5rem] text-darkGrey font-medium" >Você realmente deseja desfazer a troca ?</p>
              <div class="mt-[2rem] flex flex-row flex-wrap">
                <button @click="closeModal" class="flex flex-row justify-center items-center bg-darkGrey h-[3rem] w-[8rem] rounded-sm text-[1rem] text-white font-medium" >
                  Cancelar
                </button>
                <button class="flex flex-row justify-center items-center bg-darkGrey h-[3rem] w-[8rem] rounded-sm text-[1rem] text-white font-medium" >
                  Trocar
                </button>
              </div>
            </div>
